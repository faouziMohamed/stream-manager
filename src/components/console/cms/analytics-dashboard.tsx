...
export function AnalyticsDashboard({ initialData }: Props) {
  ...
  const totalUnpaid = (data?.paymentBreakdown ?? []).reduce(
    (s, d) => s + d.unpaid,
    0,
  );
  const totalOverdue = (data?.paymentBreakdown ?? []).reduce(
    (s, d) => s + d.overdue,
    0,
  );
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Évolution des revenus et des paiements
          </p>
        </div>
          ...
          <ResponsiveContainer width="100%" height={260}>
            ...
            >
              ...
              <Bar
                dataKey="Payé"
                stackId="a"
                fill={CHART_SEMANTIC.paid}
                radius={[0, 0, 0, 0]}
              />
              <Bar
                dataKey="En attente"
                stackId="a"
                fill={CHART_SEMANTIC.unpaid}
              />
              <Bar
                dataKey="En retard"
                stackId="a"
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              Abonnements actifs par service
            </CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer
              width="100%"
              height={Math.max(220, serviceData.length * 40)}
            >
              <BarChart
                data={serviceData}
                layout="vertical"
