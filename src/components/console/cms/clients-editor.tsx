...
import {useClients, useCreateClient, useDeleteClient, useUpdateClient,} from '@/lib/hooks/queries/use-clients.queries';
...
export function ClientsEditor({initialData}: Props) {
    ...
    const [dialog, setDialog] = useState<{ open: boolean; client?: ClientDto }>({open: false});
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground text-sm">{clients.length} client{clients.length !== 1 ? 's' : ''} au
                        total</p>
                </div>
                        ...
                        <TableBody>
                            {filtered.map((client) => (
                                <TableRow key={client.id}>
                                    ...
                                    <TableCell>
                                        <Badge variant={client.activeSubscriptionsCount > 0 ? 'default' : 'secondary'}>
                                    ...
                                    <TableCell>
                                        <div className="flex gap-1">
                                            <Button size="icon" variant="ghost" className="h-8 w-8"
                                                    onClick={() => openEdit(client)}><Pencil
                                                className="h-3.5 w-3.5"/></Button>
                                            <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive"
                                                    onClick={() => setDeleteTarget(client.id)}><Trash2
                                                className="h-3.5 w-3.5"/></Button>
                                        </div>
                                    </TableCell>
                                </TableRow>
            ...
            <Dialog open={dialog.open} onOpenChange={(o) => setDialog({open: o})}>
                <DialogContent>
                    <DialogHeader><DialogTitle>{dialog.client ? 'Modifier le client' : 'Nouveau client'}</DialogTitle></DialogHeader>
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <div className="space-y-1.5">
                            <Label>Nom *</Label>
                            <Input placeholder="Nom complet" {...register('name')} error={errors.name?.message}/>
                        ...
                        <div className="space-y-1.5">
                            <Label>Notes</Label>
                            <Textarea placeholder="Notes internes…" rows={2} {...register('notes')} />
                        </div>
                        <DialogFooter>
