...
export function GanttChart({subscriptions, fromDate, toDate}: Props) {
    ...
    while (cursor <= end) {
        ...
        const days = Math.floor(
            (visibleEnd.getTime() - cursor.getTime()) / 86_400_000,
        ) + 1;
        months.push({
        ...
        <div className="overflow-x-auto border rounded-lg">
            <div style={{minWidth: Math.max(totalDays * 4 + 200, 600)}}>
                ...
                {Array.from(serviceMap.entries()).map(([serviceName, subs]) => (
                    <div key={serviceName} className="flex border-b last:border-b-0 group hover:bg-muted/20">
                        <div className="w-48 shrink-0 px-3 py-3 text-xs font-semibold border-r truncate self-center">
                            {serviceName}
                        </div>
                        <div className="flex-1 relative" style={{height: Math.max(subs.length * 28 + 8, 40)}}>
                            {subs.map((sub, idx) => {
