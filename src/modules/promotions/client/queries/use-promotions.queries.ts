...
} from '@/modules/promotions/client/graphql/promotions.operations';
...
export function useCreatePromotion() {
  ...
  return useMutation({
    mutationFn: (input: CreatePromotionInput) =>
      gqlRequest<{ createPromotion: PromotionDto }>(CREATE_PROMOTION, {
        input,
      }).then((r) => r.createPromotion),
...
export function useUpdatePromotion() {
  ...
  return useMutation({
    mutationFn: ({ id, input }: { id: string; input: UpdatePromotionInput }) =>
      gqlRequest<{ updatePromotion: PromotionDto }>(UPDATE_PROMOTION, {
        id,
        input,
      }).then((r) => r.updatePromotion),
    onMutate: async ({ id, input }) => {
      await qc.cancelQueries({ queryKey: promotionKeys.all });
      const prev = qc.getQueryData<PromotionDto[]>(promotionKeys.all);
      qc.setQueryData<PromotionDto[]>(
        promotionKeys.all,
        (old) => old?.map((p) => (p.id === id ? { ...p, ...input } : p)) ?? []
      );
      return { prev };
    },
    onError: (err, _, ctx) => {
