...
export function PlanDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{plan ? 'Modifier la formule' : 'Nouvelle formule'}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <FormGroup label="Durée (mois)" required error={errors.durationMonths?.message}>
              <Input type="number" min={1} {...register('durationMonths')} />
            </FormGroup>
            <FormGroup label="Prix" required error={errors.price?.message}>
              <Input type="number" step="0.01" min={0} {...register('price')} />
            </FormGroup>
            <FormGroup label="Devise" required error={errors.currencyCode?.message}>
