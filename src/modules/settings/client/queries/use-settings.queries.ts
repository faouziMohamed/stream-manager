...
import {
  ...
  SET_APP_SETTING,
} from '@/modules/settings/client/graphql/app-settings.operations';
import {
...
} from '@/modules/settings/client/graphql/cloudinary-settings.operations';
import {
  ...
  SET_SMTP_SETTINGS,
  TEST_SMTP,
} from '@/modules/settings/client/graphql/smtp-settings.operations';
...
export const settingsKeys = {
  ...
  inquiries: (unreadOnly?: boolean) => ['settings', 'inquiries', unreadOnly ?? false] as QueryKey,
...
export function useDefaultCurrency(initialData?: string) {
  return useQuery({
    ...
    queryFn: () =>
      gqlRequest<{ defaultCurrency: string }>(GET_DEFAULT_CURRENCY).then((r) => r.defaultCurrency),
    initialData,
    staleTime: 5 * 60_000,
  });
}
...
export function useSmtpSettings(initialData?: SmtpSettingsDto) {
  return useQuery({
    ...
    queryFn: () =>
      gqlRequest<{ smtpSettings: SmtpSettingsDto }>(GET_SMTP_SETTINGS).then((r) => r.smtpSettings),
    initialData,
  });
}
...
export function useCloudinaryMedia(folder?: string) {
  return useQuery({
    ...
    queryFn: () =>
      gqlRequest<{ cloudinaryMedia: CloudinaryResourceDto[] }>(GET_CLOUDINARY_MEDIA, {
        folder,
...
export function useSetCloudinarySettings() {
  ...
  return useMutation({
    mutationFn: (input: CloudinarySettingsInput) =>
      gqlRequest<{ setCloudinarySettings: CloudinarySettingsDto }>(SET_CLOUDINARY_SETTINGS, {
        input,
...
export function useUploadToCloudinary() {
  return useMutation({
    mutationFn: ({ base64, filename }: { base64: string; filename?: string }) =>
      gqlRequest<{ uploadToCloudinary: CloudinaryTestResultDto }>(UPLOAD_TO_CLOUDINARY, {
        base64,
        filename,
...
export function useDeleteFromCloudinary() {
  return useMutation({
    mutationFn: (publicId: string) =>
      gqlRequest<{ deleteFromCloudinary: TestResultDto }>(DELETE_FROM_CLOUDINARY, {
        publicId,
