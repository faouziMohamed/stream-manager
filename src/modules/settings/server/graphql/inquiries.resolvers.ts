...
import { contactInquiries, inquiryReplies } from '@/lib/db/tables/subscription-management.table';
import { desc, eq } from 'drizzle-orm';
import { nanoid } from 'nanoid';
import { createLogger } from '@/lib/logger';
import { getAdminEmail, sendNotification, sendViaSMTP } from '@/lib/utils/mailer';
import type { GraphQLContext } from '@/lib/graphql/context';

const logger = createLogger('inquiries-resolvers');

export const inquiriesQueryResolvers = {
  inquiries: async (_: unknown, { unreadOnly }: { unreadOnly?: boolean }, ctx: GraphQLContext) => {
    requireAdmin(ctx);
...
export const inquiriesMutationResolvers = {
  createInquiry: async (
    ...
    try {
      ...
      if (adminEmail) {
        ...
        await sendNotification('new_inquiry', {
          ...
          html: `
              ...
              <p style="white-space:pre-wrap;margin:0">${input.message.replaceAll('<', '&lt;').replaceAll('>', '&gt;')}</p>
            `,
        });
      }
      ...
      logger.error({ err: error }, 'Failed to save contact inquiry');
      return false;
    }
  },
    ...
    if (inquiry.email) {
      const error = await sendViaSMTP({
        ...
        html: `
            ...
            <p style="white-space:pre-wrap">${body.replaceAll('<', '&lt;').replaceAll('>', '&gt;')}</p>
            ...
            <blockquote style="border-left:3px solid #eee;margin:0;padding:0 0 0 12px;color:#666;font-size:13px">
              <p style="white-space:pre-wrap">${inquiry.message.replaceAll('<', '&lt;').replaceAll('>', '&gt;')}</p>
            </blockquote>
          `,
      });
      if (error) {
  ...
  deleteInquiry: async (_: unknown, { id }: { id: string }, ctx: GraphQLContext) => {
    requireAdmin(ctx);
