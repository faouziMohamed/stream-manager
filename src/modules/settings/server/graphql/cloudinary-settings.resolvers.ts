...
} from '@/modules/settings/server/repositories/cloudinary-settings.repository';
...
export const cloudinarySettingsQueryResolvers = {
  ...
  cloudinaryMedia: async (_: unknown, { folder }: { folder?: string }, ctx: GraphQLContext) => {
    requireAdmin(ctx);
    ...
    try {
      ...
      return (result.resources ?? []).map((r: Record<string, unknown>) => ({
        ...
        folder: String(r.folder ?? r.asset_folder ?? ''),
        createdAt: String(r.created_at ?? ''),
      }));
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      logger.error({ err: error, msg }, 'cloudinaryMedia query failed');
      return [];
    }
  },
};
...
export const cloudinarySettingsMutationResolvers = {
  ...
  testCloudinary: async (_: unknown, __: unknown, ctx: GraphQLContext) => {
    ...
    try {
      ...
      logger.error({ err: error }, 'Test Cloudinary échoué');
  ...
  uploadToCloudinary: async (
    ...
    try {
      ...
      logger.error({ err: error }, 'Upload Cloudinary échoué');
      const message = error instanceof Error ? error.message : 'Erreur inconnue';
      return {
        success: false,
        message: `Échec : ${message}`,
  ...
  deleteFromCloudinary: async (
    ...
    try {
      ...
      logger.error({ err: error }, 'Suppression Cloudinary échouée');
      const message = error instanceof Error ? error.message : 'Erreur inconnue';
  ...
  replaceCloudinaryImage: async (
    ...
    try {
      ...
      const oldSubFolder = oldSegments.length >= 3 ? oldSegments.slice(1, -1).join('/') : null;
      ...
      logger.error({ err: error }, 'Remplacement Cloudinary échoué');
      const message = error instanceof Error ? error.message : 'Erreur inconnue';
      return {
        success: false,
        message: `Échec : ${message}`,
