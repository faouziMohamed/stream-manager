...
import { getProfileById } from '@/modules/accounts/server/repositories/streaming-profiles.repository';
import {
  assignSubscriptionToProfile,
  getAssignmentsByProfile,
  getAssignmentsBySubscription,
  removeAssignment,
} from '@/modules/accounts/server/repositories/profile-assignments.repository';
  ...
  assignProfile: async (
    ...
    if (input.profileId) {
      ...
      if (!profile || profile.accountId !== input.accountId) {
        throw new GraphQLError("Ce profil n'appartient pas au compte sélectionné.", {
      ...
      if (existing.length > 0 && existing[0]!.subscriptionId !== input.subscriptionId) {
        throw new GraphQLError('Ce profil est déjà assigné à un autre abonnement.', {
          extensions: { code: 'VALIDATION_ERROR' },
        });
      }
    }
