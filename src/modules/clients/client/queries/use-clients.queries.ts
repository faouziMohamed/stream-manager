...
} from '@/modules/clients/client/graphql/clients.operations';
...
export function useClients(initialData?: ClientDto[]) {
  return useQuery({
    ...
    queryFn: () => gqlRequest<{ clients: ClientDto[] }>(GET_CLIENTS).then((r) => r.clients),
    initialData,
  });
}
...
export function useClient(id: string, initialData?: ClientDto) {
  return useQuery({
    ...
    queryFn: () => gqlRequest<{ client: ClientDto }>(GET_CLIENT, { id }).then((r) => r.client),
    initialData,
    enabled: !!id,
  });
}
...
export function useCreateClient() {
  ...
  return useMutation({
    mutationFn: (input: CreateClientInput) =>
      gqlRequest<{ createClient: ClientDto }>(CREATE_CLIENT, { input }).then((r) => r.createClient),
...
export function useUpdateClient() {
  ...
  return useMutation({
    mutationFn: ({ id, input }: { id: string; input: UpdateClientInput }) =>
      gqlRequest<{ updateClient: ClientDto }>(UPDATE_CLIENT, {
        id,
        input,
      }).then((r) => r.updateClient),
...
export function useDeleteClient() {
  ...
  return useMutation({
    ...
    onMutate: async (id) => {
      await qc.cancelQueries({ queryKey: clientKeys.all });
      const prev = qc.getQueryData<ClientDto[]>(clientKeys.all);
      qc.setQueryData<ClientDto[]>(clientKeys.all, (old) => old?.filter((c) => c.id !== id) ?? []);
      return { prev };
    },
    onError: (err, _, ctx) => {
