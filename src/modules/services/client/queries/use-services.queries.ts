...
} from '@/modules/services/client/graphql/services.operations';
import {
  CREATE_PLAN,
...
} from '@/modules/services/client/graphql/plans.operations';
...
export function useServices(initialData?: ServiceDto[]) {
  return useQuery({
    ...
    queryFn: () => gqlRequest<{ services: ServiceDto[] }>(GET_SERVICES).then((r) => r.services),
    initialData,
  });
}
...
export function useService(id: string, initialData?: ServiceDto) {
  return useQuery({
    ...
    queryFn: () => gqlRequest<{ service: ServiceDto }>(GET_SERVICE, { id }).then((r) => r.service),
    initialData,
    enabled: !!id,
  });
}
...
export function useCreateService() {
  ...
  return useMutation({
    ...
    onMutate: async (input) => {
      ...
      qc.setQueryData<ServiceDto[]>(serviceKeys.all, (old) => [...(old ?? []), optimistic]);
      return { prev };
    },
    onError: (err, _, ctx) => {
...
export function useDeleteService() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { force }) =>
      toastSuccess(force ? 'Service définitivement supprimé' : 'Service archivé'),
    onSettled: () => {
...
export function usePlans(serviceId?: string, initialData?: PlanDto[]) {
  return useQuery({
    ...
    queryFn: () => gqlRequest<{ plans: PlanDto[] }>(GET_PLANS, { serviceId }).then((r) => r.plans),
    initialData,
  });
}
