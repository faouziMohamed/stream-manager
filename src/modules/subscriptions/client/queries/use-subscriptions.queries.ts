...
} from '@/modules/subscriptions/client/graphql/subscriptions.operations';
...
export function useSubscription(id: string) {
  return useQuery({
    ...
    queryFn: () =>
      gqlRequest<{ subscription: SubscriptionDto }>(GET_SUBSCRIPTION, {
        id,
...
export function useCreateSubscription() {
  ...
  return useMutation({
    mutationFn: (input: CreateSubscriptionInput) =>
      gqlRequest<{ createSubscription: SubscriptionDto }>(CREATE_SUBSCRIPTION, {
        input,
      }).then((r) => r.createSubscription),
...
export function useUpdateSubscription() {
  ...
  return useMutation({
    mutationFn: ({ id, input }: { id: string; input: UpdateSubscriptionInput }) =>
      gqlRequest<{ updateSubscription: SubscriptionDto }>(UPDATE_SUBSCRIPTION, {
        id,
        input,
      }).then((r) => r.updateSubscription),
    onMutate: async ({ id, input }) => {
      await qc.cancelQueries({ queryKey: subscriptionKeys.all });
      const prev = qc.getQueryData<SubscriptionDto[]>(subscriptionKeys.all);
      qc.setQueryData<SubscriptionDto[]>(
...
export function useDeleteSubscription() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      gqlRequest<{ deleteSubscription: boolean }>(DELETE_SUBSCRIPTION, {
        id,
      }).then((r) => r.deleteSubscription),
    onMutate: async (id) => {
      await qc.cancelQueries({ queryKey: subscriptionKeys.all });
      const prev = qc.getQueryData<SubscriptionDto[]>(subscriptionKeys.all);
      qc.setQueryData<SubscriptionDto[]>(
...
export function useRenewSubscription() {
  ...
  return useMutation({
    mutationFn: (input: RenewSubscriptionInput) =>
      gqlRequest<{ renewSubscription: SubscriptionDto }>(RENEW_SUBSCRIPTION, {
        input,
      }).then((r) => r.renewSubscription),
