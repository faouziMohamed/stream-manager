  ...
  return (
    ...
    >
        ...
        <Select onValueChange={(v) => form.setValue('planId', v)}>
          ...
          <SelectContent>
                ...
                <SelectGroup key={svc.id}>
                  ...
                  {svcPlans.map((p) => (
                    <SelectItem key={p.id} value={p.id}>
                      {p.durationMonths} mois — {formatCurrency(p.price, p.currencyCode)}
                    </SelectItem>
                  ))}
                </SelectGroup>
              );
            ...
            {promotions.map((promo) => {
              const promoPlans = plans.filter((p) => p.promotionId === promo.id);
      ...
      <FormGroup label="Notes" hint="Notes internes visibles uniquement par les administrateurs">
        <Textarea placeholder="Notes internes…" rows={2} {...form.register('notes')} />
      </FormGroup>
    </FormDialog>
  );
}
