...
import { syncOverduePayments } from '@/modules/payments/server/repositories/payments.repository';
...
export async function getDashboardStats() {
  ...
  const mrr = activeSubsWithPlans.reduce((sum, row) => {
    const monthly = Number.parseFloat(row.price) / row.durationMonths;
    return sum + monthly;
  }, 0);
...
export async function getMonthlyRevenue(months: number = 12) {
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    const revenue = rows.reduce((sum, r) => sum + Number.parseFloat(r.amount), 0);
    result.push({
      month: `${year}-${month}`,
...
export async function getSubscriptionsByService() {
  ...
  const allRows = [
    ...rows,
    ...promoRows.map((r) => ({
      serviceName: `[Bundle] ${r.serviceName}`,
  ...
  for (const r of allRows) {
    ...
    map.set(key, {
      ...
      revenue: existing.revenue + Number.parseFloat(r.planPrice),
    });
  }
