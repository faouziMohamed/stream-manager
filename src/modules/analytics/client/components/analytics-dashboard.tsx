...
import { CHART_SEMANTIC, chartColor } from '@/lib/utils/chart-colors';
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card className="card-hover">
          <CardHeader className="pb-1">
            <CardTitle className="text-muted-foreground text-sm">Revenus encaissés</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-primary text-2xl font-bold">{fmt(totalRevenue)}</p>
          </CardContent>
        </Card>
        <Card className="card-hover">
          <CardHeader className="pb-1">
            <CardTitle className="text-muted-foreground text-sm">En attente</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{fmt(totalUnpaid)}</p>
          </CardContent>
        </Card>
        <Card className="card-hover">
          <CardHeader className="pb-1">
            <CardTitle className="text-muted-foreground text-sm">En retard</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={revenueData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
              ...
              <YAxis tick={{ fontSize: 12 }} tickFormatter={String} />
              <Tooltip
                formatter={(v) => {
                  const num = typeof v === 'number' ? v : 0;
                  return [formatCurrency(num, currency), 'Revenus'];
                }}
              />
              <Line
                type="monotone"
                ...
                strokeWidth={2.5}
                dot={{ r: 3, fill: CHART_SEMANTIC.revenue }}
                activeDot={{ r: 5 }}
              />
            </LineChart>
          </ResponsiveContainer>
            ...
            <BarChart data={breakdownData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
              ...
              <Tooltip
                formatter={(v, name) => {
                  ...
                  return [formatCurrency(num, currency), label];
                }}
              />
              <Legend />
              <Bar dataKey="Payé" stackId="a" fill={CHART_SEMANTIC.paid} radius={[0, 0, 0, 0]} />
              <Bar dataKey="En attente" stackId="a" fill={CHART_SEMANTIC.unpaid} />
              <Bar
                dataKey="En retard"
                stackId="a"
                fill={CHART_SEMANTIC.overdue}
                radius={[4, 4, 0, 0]}
              />
            </BarChart>
          </ResponsiveContainer>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={Math.max(220, serviceData.length * 40)}>
              ...
              >
                ...
                <Tooltip />
                <Bar dataKey="Abonnements" radius={[0, 4, 4, 0]}>
