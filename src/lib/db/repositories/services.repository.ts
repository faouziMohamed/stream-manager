...
import {plans, promotions, promotionServices, services,} from '@/lib/db/tables/subscription-management.table';
...
export async function createPlan(input: CreatePlanInput) {
    const id = nanoid();
    const [plan] = await db.insert(plans).values({id, ...input}).returning();
