...
export async function getAllSubscriptions(clientId?: string, status?: string) {
  ...
  if (status) {
    query = query.where(
      eq(
        ...
        status as "active" | "expired" | "paused" | "cancelled",
      ),
    );
  }
...
export async function renewSubscription(input: RenewSubscriptionInput) {
  ...
  const [plan] = await db
    .select()
    .from(plans)
    .where(eq(plans.id, oldSub.planId));
  ...
  logger.info(
    { newSubId, oldSubId: input.subscriptionId },
