...
export function usePayments(
  ...
  return useQuery({
    ...
    queryFn: () =>
      gqlRequest<{ payments: PaymentDto[] }>(GET_PAYMENTS, filters).then(
        (r) => r.payments,
      ),
    initialData,
  });
...
export function useMarkPaymentPaid() {
  ...
  return useMutation({
    mutationFn: ({ id, paidDate }: { id: string; paidDate?: string }) =>
      gqlRequest<{ markPaymentPaid: PaymentDto }>(MARK_PAYMENT_PAID, {
        id,
        paidDate,
      }).then((r) => r.markPaymentPaid),
    onMutate: async ({ id }) => {
      await qc.cancelQueries({ queryKey: paymentKeys.all });
      const prev = qc.getQueryData<PaymentDto[]>(paymentKeys.all);
      qc.setQueryData<PaymentDto[]>(
