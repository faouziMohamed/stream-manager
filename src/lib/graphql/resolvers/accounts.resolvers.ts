...
export const accountsResolvers = {
    ...
    Mutation: {
        ...
        updateAccount: async (
            ...
            requireAdmin(ctx);
            if (input.maxProfiles !== undefined || input.supportsProfiles !== undefined) {
                const account = await getAccountById(id);
                if (!account) throw new GraphQLError('Compte introuvable', {extensions: {code: 'NOT_FOUND'}});
                ...
                if (input.maxProfiles !== undefined) {
                    const effectiveSupports = input.supportsProfiles ?? account.supportsProfiles;
        ...
        updateProfile: async (
            ...
            if (input.profileIndex !== undefined) {
                ...
                if (!profile) throw new GraphQLError('Profil introuvable', {extensions: {code: 'NOT_FOUND'}});
        ...
        assignProfile: async (
            ...
            if (input.profileId) {
                ...
                if (existing.length > 0 && existing[0]!.subscriptionId !== input.subscriptionId) {
                    throw new GraphQLError(
            ...
            return assignSubscriptionToProfile(input.subscriptionId, input.accountId, input.profileId);
        },
    ...
    StreamingAccount: {
        ...
        usedProfiles: (parent: { id: string }) => countAssignedProfilesByAccount(parent.id),
