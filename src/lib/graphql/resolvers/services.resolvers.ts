...
export const servicesResolvers = {
  ...
  Mutation: {
    createService: async (
      _: unknown,
      { input }: { input: { name: string; category: string; description?: string; logoUrl?: string } },
      ctx: GraphQLContext,
    ) => {
      requireAdmin(ctx);
    ...
    updateService: async (
      _: unknown,
      { id, input }: { id: string; input: { name?: string; category?: string; description?: string; logoUrl?: string; isActive?: boolean } },
      ctx: GraphQLContext,
    ) => {
      requireAdmin(ctx);
